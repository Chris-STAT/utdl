...
class WeatherForecast:
    ...
    def find_the_largest_drop(self) -> torch.Tensor:
        ...
        day_drops = torch.abs(avg_per_day.diff(1))
        return -day_drops.max()
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        """
        avg_per_day = torch.mean(self.data, dim=1).unsqueeze(1)
        diffs = self.data - avg_per_day
        max_abs_diff, idx = diffs.abs().max(dim=1)
    ...
    def max_last_k_days(self, k: int) -> torch.Tensor:
        ...
        Returns:
            tensor of size (k,)
        """
        _, max_per_day = self.find_min_and_max_per_day()
    ...
    def predict_temperature(self, k: int) -> torch.Tensor:
        ...
        return self.data[(days-k):(days+1),:].mean()
    ...
    def what_day_is_this_from(self, t: torch.FloatTensor) -> torch.LongTensor:
        ...
        dists = torch.cdist(self.data,t.unsqueeze(0),p=1)
        idx = dists.argmin()
        return idx
