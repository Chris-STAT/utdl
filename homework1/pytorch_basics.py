...
class PyTorchBasics:
    ...
    def make_it_pytorch_9(x: torch.Tensor) -> torch.Tensor:
        ...
        """
        x_row_cumsum = x.cumsum(dim=1)
        cumsum = x_row_cumsum.cumsum(dim=0)
        return cumsum
    ...
    def make_it_pytorch_10(x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.where(x >= c, x, torch.zeros_like(x))
    ...
    def make_it_pytorch_11(x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        """
        mask = x < c
        return torch.transpose(torch.nonzero(mask, as_tuple=False),0,1)
    ...
    def make_it_pytorch_extra_2(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        tolerance = 1e-3
        equal_elements = diff < tolerance
        return equal_elements.sum()
